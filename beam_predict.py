...
@endpoint(
    on_start=lambda: force_align.load_models(model_cache_dir=BEAM_VOLUME_PATH),
    name="force-align",
    cpu=2,
    memory="32Gi",
    gpu="A10G",
...
@endpoint(
    on_start=lambda: transcribe.load_models("ivrit-ai/whisper-large-v3-ct2", model_cache_dir=BEAM_VOLUME_PATH),
...
@endpoint(
    on_start=lambda: transcribe.load_models("large-v3", model_cache_dir=BEAM_VOLUME_PATH),
...
@endpoint(
    on_start=lambda: transcribe.load_models("distil-large-v3", model_cache_dir=BEAM_VOLUME_PATH),
